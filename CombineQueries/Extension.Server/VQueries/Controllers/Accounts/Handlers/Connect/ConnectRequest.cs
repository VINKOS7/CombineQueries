...
public record ConnectRequest : IRequest<ConnectResponse>
{
    ...
    // Забыть хайперы при подключении. Собранный однажды url дальше уходит одним запросом /h/,
    // и повторный прогон меряет уже не сборку - для теста это надо уметь сбрасывать.
    // Уважается ТОЛЬКО в Development: на релизе стёрло бы накопленное живым миром.
    [JsonProperty("resetHypers")] public bool ResetHypers { get; set; }
