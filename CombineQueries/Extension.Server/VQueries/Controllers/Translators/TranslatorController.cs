...
public class TranslatorController : Controller
{
    ...
    [AllowAnonymous][HttpGet("/h/{jump:int}/{count:int}/{sign:int}")] public Task<HyperResponse> HyperRange(int jump, int count, int sign) => _mediator.Send(new HyperRequest { Value = jump, Count = count, Sign = sign });

    [AllowAnonymous] [HttpGet("/hd/{fragment:int}/{shortened:int}/{sign:int}")] public Task<HeadResponse> HeadFrom(int fragment, int shortened, int sign) => _mediator.Send(new HeadRequest { Fragment = fragment, Base = shortened, Sign = sign });
