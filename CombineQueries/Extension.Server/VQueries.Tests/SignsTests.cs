...
//
// Части разделены на наборы, и набор закреплён за клиентом: спутать клиентов нельзя. При наборе из
// одной части кольцо вырождается в одну повторённую часть - это и есть номер клиента.
//
// Отказ по своей части значит, что набор отдали другому: клиентов больше, чем наборов, и самый
// залежавшийся вытесняется. Тест это терпит и идёт к следующей части.
//
...
public class SignsTests
{
    ...
    public async Task RingIsReplacedWhenSpent()
    {
        ...
        Assert.That(ring.Fresh, Has.Length.EqualTo(signs.Length), "новое кольцо другой длины");

        // Новое кольцо обязано остаться в наборе клиента: иначе его части начнут попадать к соседям.
        Assert.That(ring.Fresh.Distinct(), Is.SubsetOf(signs.Distinct()), "новое кольцо вышло за набор клиента");
